...
import picluster
import endpoint
# This scipt runs on a raspberry pi and it pings the Azure service every 60 seconds to keep it updated on the pi behavior.
...
while True: 
    try:
        ...
        # setup heartbeat message, which doesn't modify current user or task or command.
        r.ip_address = ip
...
while True:
    try:
        ...
        errorType, value, traceback = sys.exc_info()
        print("network error: {}:{}, perhaps we don't have an ip address right now, so sleep and try again later...".format(str(errorType), str(value)))
